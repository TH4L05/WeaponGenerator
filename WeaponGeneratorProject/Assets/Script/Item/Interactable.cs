...
public class Interactable : MonoBehaviour
{
    ...
    public void EventSpawnNewWeapon()
    {
        ...
        weapon.transform.localPosition = specialTransforms[0].position;
    }

    public void ItemSpawnAmount(int amount)
    {
        this.amount = amount;
    }

    public void EventEquipWeapon()
    {     
        var weapon = gameObject.GetComponentInChildren<Weapon>();
        if (weapon.Data.level > Game.instance.player.Level)
        {          
            return;
        }
        Game.instance.player.WeaponController.EquipWeapon(gameObject);
    }
    ...
    {
        var rb = Game.instance.player.ItemOnFocus.gameObject.GetComponent<Rigidbody>();
    }
}
